...
class Dial:
   ...
   def __init__(self, image, frameImage, x=0, y=0, w=0, h=0):
       ...
       dial_width = self.frameImage.get_width()
       dial_height = self.frameImage.get_height()
       self.dial = pygame.Surface((dial_width, dial_height))
...
class Color_dial(Generic):
   """
   Colored battery dial.
   """
   def __init__(self, x=0, y=0, w=0, h=0):
       """
       Initialise dial at x,y.
       Default size of 300px can be overidden using w,h.
       """
       self.font1 = pygame.font.Font(None, 60)
       self.font2 = pygame.font.Font(None, 40)
       Generic.__init__(self, x, y, w, h)
       self.frameImage = pygame.image.load('images/Treatment 3/gauge back new.png').convert()
   def update(self, screen, angleX, bar):
       """
       Called to update a Battery dial.
       "angleX" is the input.
       "screen" is the surface to draw the dial on.
       ...
       if angle > 40:
           color = (0,255,0)
           self.image = pygame.image.load('images/Treatment 3/AirSpeedNeedleGreen.png').convert()
       elif angle > 20:
           color = (150,100,0)
           self.image = pygame.image.load('images/Treatment 3/AirSpeedNeedleYellow.png').convert()
       else:
           color = (255,0,0)
           self.image = pygame.image.load('images/Treatment 3/AirSpeedNeedleRed.png').convert()
       ...
       image1 = self.font1.render(str(int(angle)), 0, color)
       image2 = self.font2.render(str(bar)+'%', 0, color2)
       tmpImage1 = self.clip(image1, 0, 0, 0, 0, 0, -35)
       tmpImage2 = self.clip(image2, 0, 0, 0, 0, 0, -35)
       self.overlay(self.frameImage, 0, 0)
       self.overlay(tmpImage1, -10, 95)
       self.overlay(tmpImage2, 106, -70)
       ...
       side_bar = Rect(screen.get_width() - 27, screen.get_height() - bar - 40, 12, bar)
       pygame.draw.rect(screen, color2, side_bar)


class Middle_dial(Generic):
   """
   Colored battery dial.
   """
   def __init__(self, x=0, y=0, w=0, h=0):
   ...
   def update(self, screen, angleX, bar):
       ...
       angleX -= 135
       if angle > 40:
           self.image = pygame.image.load('images/Treatment 2/arrow green.png')
       elif angle > 20:
           self.image = pygame.image.load('images/Treatment 2/arrow yellow.png')
       else:
           self.image = pygame.image.load('images/Treatment 2/arrow red.png')
       self.image = self.clip(self.image, 0, 0, 0, 0, 0, -35)
       color1 = (120, 200, 80)

       image1 = self.font1.render(str(int(angle)), 0, color1)
       image2 = self.font2.render(str(bar)+'%', 0, color1)
       tmpImage1 = self.clip(image1, 0, 0, 0, 0, 0, -35)
       tmpImage2 = self.clip(image2, 0, 0, 0, 0, 0, -35)
       self.overlay(self.frameImage, 0, 0)
       self.overlay(tmpImage1, -10, 95)
       ...
       side_bar1 = Rect(screen.get_width() - 27, screen.get_height() - 66, 12, 20)
       side_bar2 = Rect(screen.get_width() - 27, screen.get_height() - 86, 12, 20)
       side_bar3 = Rect(screen.get_width() - 27, screen.get_height() - 146, 12, 60)
       walk_bar = Rect(screen.get_width() - 32, screen.get_height() - bar - 46, 22, 5)
       pygame.draw.rect(screen, (255,0,0), side_bar1)
       pygame.draw.rect(screen, (150,100,0), side_bar2)
       pygame.draw.rect(screen, (0,255,0), side_bar3)
       pygame.draw.rect(screen, color1, walk_bar)
...
class Digits(Dial):
   ...
   def __init__(self, x=0, y=0, w=0, h=0):
       ...
       Dial.__init__(self, None, self.frameImage, x, y, w, h)
   def update(self, screen, angleX, bar):
       """
       Called to update a Generic dial.
       "angleX" and "angleY" are the inputs.
       "screen" is the surface to draw the dial on.
       """
       image1 = self.font.render(str(angleX//1), 1, (120, 200, 80))
       image2 = self.font.render(str(bar)+'%', 1, (120, 200, 80))
       tmpImage1 = self.clip(image1, 0, 0, 0, 0, 0, -35)
       tmpImage2 = self.clip(image2, 0, 0, 0, 0, 0, -35)
       self.overlay(self.frameImage, 0, 0)
       self.overlay(tmpImage1, 0, 0)
       self.overlay(tmpImage2, 0, 80)
       self.dial.set_colorkey(0xFFFF00)
       screen.blit(pygame.transform.scale(self.dial, (self.w, self.h)), self.pos)
...
#    """
#    def __init__(self, port, baud, testing=1):
#       """
...
#          print(" Opens SERIAL_DEVICE and listens for telemetry data.")
